...
// ─── MongoDB Connection ────────────────────────────────────────────────────
mongoose.connect("mongodb+srv://safiswati10_db_user:Onyxtaffy%40123@cluster0.xajhmc0.mongodb.net/payments")
...
app.get('/api/payments', authMiddleware, async (req, res) => {
  try {
    ...
    const [payments, total] = await Promise.all([
      Payment.find(query).sort({ created_at: -1 }).skip(skip).limit(parseInt(limit)),
      Payment.countDocuments(query)
    ]);

    return res.json({ payments, total, page: parseInt(page), pages: Math.ceil(total / parseInt(limit)) });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/payments/stats', authMiddleware, async (req, res) => {
  try {
    ...
    const todayCount = await Payment.countDocuments({ created_at: { $gte: today } });
    ...
    return res.json({ total, totalAmount, todayCount, topCountries: countries });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});
